...
class asterisk_server(osv.osv):
    ...
    _columns = {
        ...
        'number_of_digits_to_match_from_end': fields.integer(
            ...
            help='In several situations, the Asterisk-OpenERP connector will '
                 'have to find a Partner in OpenERP from a phone number '
                 'presented by the calling party. As the phone numbers '
                 'presented by your phone operator may not always be '
                 'displayed in a standard format, the best method to find the '
                 'related Partner in OpenERP is to try to match the end of '
                 'the phone numbers of the Partners in OpenERP with the N '
                 'last digits of the phone number presented by the calling '
                 'party. N is the value you should enter in this field.'
        ),
    ...
    def _check_validity(self, cr, uid, ids):
        for server in self.browse(cr, uid, ids):
            ...
            if (server.number_of_digits_to_match_from_end > 20
                    or server.number_of_digits_to_match_from_end < 1):
                raise osv.except_osv(
                    _('Error :'),
                    _("You should set a 'Number of digits to match from end' "
                      "between 1 and 20 for the Asterisk server '%s'"
                      % server.name)
                )
            for check_string in [
                    dialplan_context,
    ...
    def _reformat_number(self, cr, uid, erp_number, ast_server, context=None):
        ...
        logger.notifyChannel(
            'click2dial',
            netsvc.LOG_DEBUG,
            'Number before reformat = %s' % tmp_number
        )
        ...
        logger.notifyChannel(
            'click2dial',
            netsvc.LOG_DEBUG,
            'Number after removal of special char = %s' % tmp_number
        )
        ...
        logger.notifyChannel(
            'click2dial',
            netsvc.LOG_DEBUG,
            'Country prefix = %s' % country_prefix
        )
        if country_prefix == tmp_number[0:len(country_prefix)]:
            ...
            logger.notifyChannel(
                'click2dial',
                netsvc.LOG_DEBUG,
                'International prefix = %s - Number with international '
                'prefix = %s' % (international_prefix, tmp_number)
            )
    ...
    def _get_calling_number(self, cr, uid, context=None):
        ...
        try:
            ...
            logger.notifyChannel(
                'click2dial',
                netsvc.LOG_DEBUG,
                "Result of Status AMI request: %s" % list_chan
            )
            for chan in list_chan.values():
                ...
                if (chan.get('ChannelState') == '4'  # 4 = Ring
                    ...
                    logger.notifyChannel(
                        'click2dial',
                        netsvc.LOG_DEBUG,
                        "Found a matching Event in 'Ring' state"
                    )
                ...
                if (chan.get('ChannelState') == '6'  # 6 = Up
                    ...
                    logger.notifyChannel(
                        'click2dial',
                        netsvc.LOG_DEBUG,
                        "Found a matching Event in 'Up' state"
                    )
                    calling_party_number = chan.get('CallerIDNum')
                    break
                # Compatibility with Asterisk 1.4
                if (chan.get('State') == 'Up'
    ...
    _columns = {
        ...
        'asterisk_server_id': fields.many2one(
            ...
            help="Asterisk server on which the user's phone is connected. "
                 "If you leave this field empty, it will use the first "
                 "Asterisk server of the user's company."
        ),
    }
...
class res_partner_address(osv.osv):
    ...
    _columns = {
        'phone_e164': fields.function(
            ...
            multi="e164",
            store={
                'res.partner.address': (lambda self, cr, uid, ids, c=None:
                                        ids, ['phone'], 10),
            }
        ),
    ...
    def get_partner_from_phone_number(self, cr, uid, presented_number,
                                      context=None):
        ...
        ast_server_obj = self.pool.get('asterisk.server')
        ast_server = ast_server_obj._get_asterisk_server_from_user(
            cr, uid, context=context
        )
        nr_digits_to_match_from_end = (
            ast_server.number_of_digits_to_match_from_end
        )
        ...
        logger.notifyChannel(
            'click2dial',
            netsvc.LOG_DEBUG,
            "Will search phone and mobile numbers in res.partner ending with "
            "'%s'" % end_number_to_match
        )
        ...
        if len(res_ids) > 1:
            logger.notifyChannel(
                'click2dial',
                netsvc.LOG_WARNING,
                u"There are several partners addresses (IDS = %s) with a "
                u"phone number ending with '%s'"
                % (str(res_ids), end_number_to_match)
            )
        if res_ids:
            entry = self.read(
            ...
            logger.notifyChannel(
                'click2dial',
                netsvc.LOG_DEBUG,
                u"No match for end of phone number '%s'" % end_number_to_match
            )
            return False
