...
class BackendVoicentCallLine(models.Model):
    ...
    reply_ids = fields.One2many(
        "backend.voicent.call.line.reply", "line_id", string="Replies"
    )
    contact_ids = fields.One2many(
        "backend.voicent.call.line.contact", "line_id", string="Contact Info"
    )
...
class BackendVoicentCallLineReply(models.Model):
    ...
    reply_field = fields.Selection(
        VOICENT_REPLY, string="Voicent Reply Field", required=True
    )
