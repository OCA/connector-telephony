...
#         'size': 255,
#         'type': 'char',
#         'help': 'Enter the verification code that you get in your
#                  verification sms'
#     }
# }
...
class verifycode(orm.TransientModel):
    ...
    def checkcode(self, cr, uid, data, context):
        ...
        gate = pooler.get_pool(cr.dbname).get('sms.smsclient').browse(
        ...
        if gate.code == data['form']['code']:
            pooler.get_pool(cr.dbname).get('sms.smsclient').write(cr, uid,
                                                                  [data['id']],
                                                                  {'state':
                                                                   'confirm'})
        else:
