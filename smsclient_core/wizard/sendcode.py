...
class sendcode(orm.TransientModel):
    ...
    def send_code(self, cr, uid, data, context):
        key = md5(time.strftime('%Y-%m-%d %H:%M:%S') +
                  data['form']['smsto']).hexdigest()
        ...
        if gate.state not in ('new', 'waiting'):
            raise osv.except_osv(_('Error'), _(
                'Verification Failed. Please check the Server Configuration!'))

        pooler.get_pool(cr.dbname).get('sms.smsclient').write(
            cr, uid, [data['id']], {'state': 'waiting',
                                    'code': msg})
        return {}
...
#                       'arch': form,
#                       'fields': fields,
#                       'state': [('end', 'Cancel'), ('send', 'Send Code')]
#                       }
#        },
#        'send': {
#            'actions': [send_code],
