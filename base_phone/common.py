...
def convert_phone_field(value, country_code):
    _logger.debug("convert_phone_field value=%s country=%s", value, country_code)
    try:
    ...
    for field in fields_to_convert:
        ...
        if country_key:
            if country_key in loc_vals:
                ...
                country = self.env["res.country"].browse(int(vals[country_key]))
            else:
                country = self[country_key]
        if partner_key and not country:
            if partner_key in loc_vals:
                partner = self.env["res.partner"].browse(int(vals[partner_key]))
            else:
