...
class VozTelecomAPICallback(http.Controller):
    def notify_users(self, users, response, caller):
        for user in users:
            ...
            body = (
                ...
                + (str(caller.id or ""))
                + "&action="
                + str(request.env.ref("contacts.action_contacts").id)
                + "&model=res.partner&view_type=form&cids=1'>"
                + ("Abrir" if caller else "Crear")
                + " Cliente</a>"
            )
            if ch:
                ch.message_post(
    ...
    def voz_telecom_callback(self, *args, **post):
