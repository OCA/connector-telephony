...
class VozTelecomAPICallback(http.Controller):
    @http.route('/vtcrm/callback/', type='json', auth='none', methods=["POST"],
                csrf=False)
    def authenticate(self, *args, **post):
        ...
        try:
            ...
            db = 'VozTelecom'
        uid = request.env["res.users.apikeys"]._check_credentials(scope="rpc",
                                                                  key=password)
        ...
        caller = request.env['res.partner'].search(['|',
                                                    ('phone_no_spaces', 'ilike',
                                                     response[
                                                         'displayedExternalNumber']),
                                                    ('mobile_no_spaces', 'ilike',
                                                     response[
                                                         'displayedExternalNumber'])
                                                    ])
        ...
        if response['isIncoming'] == 'true' and response['state'] == 'ringing':
            for user in users:
                ...
                ch = request.env['mail.channel'].sudo().search(
                ...
                if caller.id:
                    ...
                    body = "<h2>Llamada entrante de " \
                           + (caller.name or response['displayedNumber']) + \
                           "</h2><a href='/web#id=&action="+request.env.ref('contacts.action_contacts')+"&model=res.partner" \
                           "&view_type=form" \
                           "&view_type=form&cids=1'> " \
                           "Crear Cliente</a>"
                ...
                if response['isIncoming'] == 'true':
                    for user in users:
                        user.notify_default(message='Llamada de ' + (
                                caller.name or response[
                            'displayedNumber']) + ' respondida por ' +
                                                    user_id.name if user_id else
                        response['extension'])
                else:
                    user_id.notify_default(
                        message='Llamada a ' + (caller.name or response[
                            'displayedNumber']) + ' establecida')
                data['user_id'] = user_id.id
                data['voz_telecom_state'] = 'talking'
            if phonecall.voz_telecom_state == 'talking' and response[
                'state'] == 'dropped':
                user_id.notify_default(
                    message='Llamada de ' + (caller.name or response[
                        'displayedNumber']) + ' finalizada')
