...
class ResPartner(models.Model):
	...
	phone_no_spaces = fields.Char(compute="_phone_no_spaces", store=True)
	mobile_no_spaces = fields.Char(compute="_mobile_no_spaces", store=True)

	def api_phone_call(self):
		api_controller = self.env['voztelecom.config'].search([])[0]
		api_controller.api_trigger_call(self.env.user.phone or self.env.user.mobile, self.phone)

	def api_mobile_call(self):
		api_controller = self.env['voztelecom.config'].search([])[0]
		api_controller.api_trigger_call(self.env.user.phone or self.env.user.mobile, self.mobile)
